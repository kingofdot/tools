...
const PATTERNS = [
  ...
  // 법명 prefix 후보 — "○○법", "○○법 시행령", "○○법 시행규칙",
  //   또는 노트 컨텍스트 의존(시행령/시행규칙 단독)
  // ② "○○법[ 시행령|시행규칙] N조[, N조]+" — 콤마 다중
  {
    re: /[가-힣]{1,14}법(?:\s+(?:시행령|시행규칙))?\s+제?\s*\d+\s*조(?:의\s*\d+)?(?:\s*제?\s*\d+\s*항)?(?:\s*제?\s*\d+\s*호)?(?:\s*[,，、]\s*제?\s*\d+\s*조(?:의\s*\d+)?(?:\s*제?\s*\d+\s*항)?(?:\s*제?\s*\d+\s*호)?)+/g,
    cls: 'tag-law',
    type: 'law-multi',
  },
  // ②.5 "시행령 N조[, N조]+" / "시행규칙 N조[, N조]+" — 단독 (콤마 다중)
  {
    re: /(?:시행령|시행규칙)\s+제?\s*\d+\s*조(?:의\s*\d+)?(?:\s*제?\s*\d+\s*항)?(?:\s*제?\s*\d+\s*호)?(?:\s*[,，、]\s*제?\s*\d+\s*조(?:의\s*\d+)?(?:\s*제?\s*\d+\s*항)?(?:\s*제?\s*\d+\s*호)?)+/g,
    cls: 'tag-law',
    type: 'law-multi',
  },
  // ③ "○○법[ 시행령|시행규칙] 제N조 ..." — 법명 + "제" 있는 형태
  {
    re: /[가-힣]{1,14}법(?:\s+(?:시행령|시행규칙))?\s+제\s*\d+\s*조(?:의\s*\d+)?(?:\s*제\s*\d+\s*항)?(?:\s*제\s*\d+\s*호)?/g,
    cls: 'tag-law',
    type: 'law',
  },
  // ④ "○○법[ 시행령|시행규칙] N조 ..." — 법명 + "제" 없는 형태
  {
    re: /[가-힣]{1,14}법(?:\s+(?:시행령|시행규칙))?\s+\d+\s*조(?:의\s*\d+)?(?:\s*\d+\s*항)?(?:\s*\d+\s*호)?/g,
    cls: 'tag-law',
    type: 'law',
  },
  // ④.5 "시행령 [제]N조 ..." / "시행규칙 [제]N조 ..." — 단독
  {
    re: /(?:시행령|시행규칙)\s+제?\s*\d+\s*조(?:의\s*\d+)?(?:\s*제?\s*\d+\s*항)?(?:\s*제?\s*\d+\s*호)?/g,
    cls: 'tag-law',
    type: 'law',
  },
  // ⑤ "제N조 제N항" — 법명/접미 없이, "제" 있는 형태
  {
